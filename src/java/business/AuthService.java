...
public class AuthService {
    ...
    public AuthService() {
        this.authDAO = new DerbyAuthDAO();
        this.logDAO  = new PostgresQLDAO();
    }
    ...
    public User login(String username, String plainPassword, String ipAddress) {
        ...
        User user = authDAO.authenticate(sanitizedUsername, plainPassword);

        if (user != null) {
            LOGGER.info("Login SUCCESS: " + sanitizedUsername + " [" + user.getRole() + "] from " + ipAddress);
            logDAO.log(sanitizedUsername, "LOGIN_SUCCESS", ipAddress, user.getRole(), "AuthService");
        } else {
    ...
    public void logUnauthorizedAccess(String username, String role,
    ...
    public DerbyAuthDAO getAuthDAO() {
        return authDAO;
    }

    public PostgresQLDAO getLogDAO() {
        return logDAO;
    }
}